...
interface Props { sprintId: string; currentGoal: string; }
...
export default function SprintGoalForm({ sprintId, currentGoal }: Props) {
  ...
  return (
    <div style={{
      ...
      <div className="summary-label" style={{ marginBottom: 14 }}>
        <span>🎯</span> Sprintdoel
      </div>
      <textarea
      ...
      <div className="doel-actions">
        <button className="btn-primary" onClick={handleSave} disabled={status === "saving" || !goal.trim()}>
          {status === "saving" ? "Opslaan…" : "Opslaan"}
        </button>
        {status === "saved" && <span style={{ fontSize: 13, color: "var(--green)" }}>✓ Opgeslagen</span>}
        {status === "error" && <span style={{ fontSize: 13, color: "var(--red)" }}>Opslaan mislukt</span>}
        <a href="/sprint" className="btn-secondary">← Terug</a>
      </div>
      {currentGoal && status !== "saved" && (
        <p style={{ marginTop: 16, fontSize: 12, color: "var(--muted)", borderTop: "1px solid var(--border)", paddingTop: 14 }}>
          Huidig: <em style={{ color: "var(--text-2)" }}>{currentGoal}</em>
        </p>
      )}
    </div>
  );
